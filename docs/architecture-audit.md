...
AUD-022 through AUD-056 came out of the 2026-08-06 deep audit and are tracked, with their fixes, in
[Deep Audit 2026-08-06](deep-audit-2026-08-06.md) rather than duplicated into the table below. All
thirty-five are fixed across seven passes: AUD-022 to AUD-033 (first), AUD-034 to AUD-037 (second),
AUD-038 to AUD-039 (third), AUD-040 to AUD-043 (fourth, stats paging), AUD-044 to AUD-045 (fifth,
combat ownership and account identity), AUD-046 to AUD-055 (sixth, the previously unread
controllers), and AUD-056 (EnemySpawner dead branches).

**Compile and test verification completed 2026-08-07.** The full edit-mode suite runs green in
Unity 6000.5.6f1 - 74 tests, 74 passed, 0 failed, with zero `error CS` diagnostics across
`Assembly-CSharp` and `Assembly-CSharp-Editor`. That covers every deep-audit pass and the
architecture-register work below. See the verification section of the deep-audit document for the
per-fixture breakdown. **Playtest verification is still outstanding** - passing tests mean the code
