...
| Vehicles/traffic | `TrafficManager`, `VehicleController`, `VehicleMove`, `BehaviorVehicleLawnmower` | Spawns and moves traffic/vehicles and handles special vehicle behavior. | Candidate for pooling and state-based behavior similar to enemy spawning. |
...
Target direction: document the shot lifecycle separately, then move toward a single shot result event that UI, stats, audio, progression, and game rules can consume.
