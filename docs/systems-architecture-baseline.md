...
| Input | `PlayerControls`, `PlayerControlsProvider`, `PlayerInputReader`, `TouchInputController`, screen-specific touch controllers | Provides generated input bindings, player gameplay input intent, and screen-specific touch/menu input routing. | First input-reader slice is in place for `PlayerController`. Mobile/menu touch still needs migration to Input System on-screen controls, EnhancedTouch where gestures remain, and standard UI input modules. |
