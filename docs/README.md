...
- [Player Input Smoke Validation](player-input-smoke-validation.md) - per-behavior keyboard/gamepad/mobile/menu/debug-input checklist, each row marked with validation type and status; a validation gate for future input-ownership work, not a record of manual/device validation performed.
- [UI and Menu Architecture](ui-input-architecture.md) - menu input/wiring/scaling baseline, the single-route target, the phased migration, and per-screen smoke checks.
...
- [Phase 1b — Match-stats Owner](phase-1b-match-stats-owner-plan.md) - the slice plan: `GameStats` measured at 147 mentions across 32 files with zero authored serialized values, a one-bool seam to `basketball`, and a plain owner in `Level5.Core` behind a delegating facade.
