...
| AUD-013 | Player identity/role flags are duplicated with no single owner. | A spawn/mode path can set one copy and forget the other. | Make `PlayerIdentifier` the single source of truth for identity/role flags. | **Closed 2026-08-31.** Controllers read identity/role from `PlayerIdentifier`; the basketball object no longer carries a second, hand-synced `PlayerIdentifier` at all - `IBasketballRuntime`, implemented on `BasketBall`/`BasketBallAuto`, is bound explicitly by `SpawnCoordinator.GiveBall`. See the AUD-013 row in `architecture-audit.md` for the full writeup. |
...
4. **Done 2026-08-02 (partial); structural half closed 2026-08-31.** `BasketBall.cs`, `groundcheck.cs`, and `AutoPlayerController.cs` now read `.isCpu`/`.isDefensivePlayer` through `PlayerIdentifier` instead of controller duplicate fields where runtime behavior depends on identity/role (AUD-013); the controller-owned duplicate fields and `PlayerIdentifier.bid`/`bsid` were deleted the same day (2026-08-14 remainder pass). The remaining two-`PlayerIdentifier`-instances-per-slot structure is now closed too: `IBasketballRuntime` is implemented directly on `BasketBall`/`BasketBallAuto` and bound explicitly by `SpawnCoordinator.GiveBall` immediately after `Instantiate`, and `PlayerIdentifier` was removed from both `basketball.prefab` and `basketballAuto.prefab` via `PrefabUtility`. `groundcheck.cs`'s ball-attached `GroundCheck` instance (a child of the basketball object, not the player) was a previously-undocumented consumer of the ball's own duplicate `PlayerIdentifier`, found via a PlayMode test regression after the prefab edit; it now resolves through `IBasketballRuntime` when no `PlayerIdentifier` is found in its parent hierarchy, leaving its player/autoPlayer-attached behavior untouched. Full writeup in `architecture-audit.md`'s AUD-013 row.
